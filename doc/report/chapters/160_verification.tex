...
this chapter. Furthermore the throughput is compared to a CPU based solution.
...
First the three FPGA implementations are put to contrast.
...
image lines are buffered on the FPGA. The HLS 256bit theoretical result is
approximately 11 times faster than the 8bit version. This is due to the fact
that... \todo{WHY???}. Finaly the VHDL implementation yields the highest
throughput for images larger or equal to 1080p. This was to be expected for two
reasons. First, the VHDL wallis filter core can process pixels at one pixel per
clock cycle which is more than the 8bit HLS version is capeable of \todo{true?}.
...
\begin{table}[tb!]
    ...
    \begin{tabular}{l l l l l}
        ...
        \midrule
        HLS 8bit   & 0.17  &       & 0.168 & 0.161 \\
        HLS 256bit & 11.3 & 11.1 & 11.0 & 10.9 \\
        VHDL       & 0.89  & 1.291 & 2.348 & 4.115 \\
...
The increase in throughput over image size can be explained by the way the
...
\ref{eq:theomaxb} shows the throughput (for derivation refer to appendix \ref{app:derivations:theomax}).
...
implementation with highest throughput (VHDL implementation) is put to contrast
with a CPU based computation. Before this can be compared the maximum possible
throughput of the VHDL implementation is calculated because the communication is
the limiting factor.

\begin{align}
    b  & = \frac{i_h}{w_l (i_h-w_l+1)}
    \label{eq:theomaxvhdlwallisshort}
\end{align}

Equation \ref{eq:theomaxvhdlwallisshort} is used to calculate the maximum
possible throughput for each of the test images (for derivation refer to appendix \ref{app:derivations:theomaxvhdlwallis}). The CPU programm was run
multiple times for each image and the mean throughput calculated. Figure 
\ref{fig:theoreticalmax} and table \ref{tab:throughputcompare} show the results.
