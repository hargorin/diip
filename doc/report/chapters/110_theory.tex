...
Pipeline is used if not all data for processing is available in the loop. Figure \ref{fig:p_pipeline_unroll} (pipeline) shows that only one read access per clock can be made. This results in a latency of one clock. The other operations can be parallelized. This will lead to a processing of the loop of 5 clock cycles. The throughput could therefore be increased.

\item[Unroll]\hfill \\ 
This directive allows the entire for loop to be parallelized. This is only possible if all data is available at the beginning of the for loop. Figure \ref{fig:p_pipeline_unroll} shows that the processing of the loop only requires 3 clock cycles and the throughput could be increased in comparison to \texttt{pipeline}.
\end{description}
...
\todo[inline]{NH: you explained Pipeline/Unroll way better at ECC than in these
two
blocks of text. pipeline and unroll sound almost the same in this text.}
