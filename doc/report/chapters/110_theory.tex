...
about image processing and filtering as well as communication via Ethernet. What
kind of image operations are possible and the Wallis filter are explained in the
...
They calculate the result and store it at the reference point in the target
image. Neighborhood operations are often used in convolutional filters. These
...
image. For example, this is possible with the Sobel filter.
...
problem occurs in every image (see figure \ref{fig:image_handling}). Because the filter protrudes beyond the original image.
...
\begin{tabular}{rl}
    ...
    $\mu_{g}        =$ & mean value of the destination image \\
    $\sigma_{g}^{2} =$ & variance value of the destination image \\
    $b              =$ & brightness factor \\
    $c              =$ & contrast factor \\
\end{tabular} \\
