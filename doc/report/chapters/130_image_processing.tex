...
The operation is row based. This means that each new row of an image has the initialization procedure.
...
First, the accuracy of the division i commented because resources can already be
...
tested and the accuracy of the intermediate variables adjusted. The data types
ap\_[u]int and
...
calcualation, as well as the Wallis filter. They are shown as a block diagram and are briefly explained.
...
The pixel to be calculated is connected to the IP core via AXI4-Stream. The constant parameters are connected to the \texttt{controller} and are sent from the computer. \\
