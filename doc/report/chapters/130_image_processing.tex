...
\todo[inline]{NH: Describe why the mean and var calculation is so complicated.
The
important message here is to show that it is done that way to be able to 1.
iterate and 2. calculate mean and var at the same time. At least say it is
explained later in ...}
\todo[inline]{NH: Why number 1/441? missing context here}
\todo[inline]{NH: The sum block with feedback to input is not self-explanatory,
must be explaind, else leads to confusion}
...
\todo[inline]{NH: Perhaps make a subsubsection* explaining WIN\_SIZE and
WIN\_LENGTH to omit sqrt stuff?}
...
\begin{table}[tb!]
    ...
    \label{tab:datatypes}
\end{table}
\todo[inline]{NH: Cool table! But what do these variables represent? And why do
some have no min/max Values?}
...
\todo[inline]{NH: Source of division latency?}
...
\todo[inline]{NH: Why is the input stream only 8bit? Could be wider. Explain}
...
\todo[inline]{NH: Perhaps mention in the beginning of image processing that the
window length used is 21?}
\begin{minipage}{\textwidth}
...
\todo[inline]{NH:Again, for leserfuehrung mention at the beginning of image
processing that first HLS and then VHDL and why. Else this comes too sudden.}
...
\todo[inline]{NH: A simulation result showing the output of dir\_shift\_reg and
sum\_diff would clarify what the intention is. Perhaps with 3x3 window and
valid signal}
\begin{figure}[tb!]
    \centering
...
\todo[inline]{NH: What two implementation of the mean and the variance are you
talking about here?}
\todo[inline]{NH: less than one, play a role in what}
...
\todo[inline]{NH: is it not a multiplication of two fractionals and not
intergers?}
...
\todo[inline]{NH: Why are the 8 most sigificants remover? It would make sense
if the 8 least significant bits were removed}
\todo[inline]{NH: Why exactly can the decimal place change with each
calculation?}
\todo[inline]{NH: Again, multiplication of integers or fractionals?}
...
\todo[inline]{NH: If the divider core was capable of pipelining, the latency
can be larger than 21. Altough if the throughput is less than 1 px every 21
clks, then we would have a problem. Your explanation here is wrong }
