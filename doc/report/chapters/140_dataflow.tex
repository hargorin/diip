...
parts: the communication and control parts. But before diving into them the
...
a Tx FiFo of the UFT transmitter. This causes latency and high resource usage.
...
received data coming from the Ethernet line with low latency. The controller
...
preceding project the p5 project report can be consulted \cite{p5report}.
...
data sent is not acknowledged hence the sender has no feedback whether the data
was received or not. This is one reason a custom session protocol (\gls{uft})
was introduced in the preceding project. It provides command packets for data
...
Wallis filter however, requires its input data to be column wise with the top
...
coming from the UFT core and the red ``read'' arrow represents the order the
...
marks the new column of the neighborhood to be sent to the Wallis filter.
...
This progression is made for each column of the input image until the last
column was processed. Then scenario B) comes to play. Moving the neighborhood
...
solve:
\begin{itemize}
    \item Send the image data in the right order (column wise with
    $WINDOW\_LENGTH$ sized columns)
...
been processed and configure the Wallis filter with the parameters coming from
the UFT core.
...
code.
The code in file \texttt{controller.cpp} implements the state machine behavior.
The state machine code is very straightforward. More work went into the memory
...
realized. There are two buffers, each of the size of $WINDOW\_LENGTH \cdot
...
$WINDOW\_LENGTH$. Therefore it counts the current column. The second counter
...
Every time a pixel is read or written a corresponding counter is incremented.
...
The same applied to the sending of data. It is first checked whether the output
...
in the output buffer. This increases the resource usage.
...
requirements, how the core was realized, the hurdles across the way and a
...
required a new memory structure. These two aspects form the requirements of the
new controller core.
...
synthesis results). Every line represents an input image line and every column an
input
image column. The idea is to have a full block RAM for each line which is why
...
pointer points to the current image column that is sent to the Wallis filter.
...
that now the behavior was not described in C/C++ language but in VHDL. The
total
...
Performance wise, there is the expected increase in throughput. Output pixels
